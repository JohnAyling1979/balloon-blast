import styles from './Header.module.css';

function Header() {
  return (
    <div className={styles.root}>
    <h1 className={styles.header}>
      <span className={styles.c1}>B</span>
      <span className={styles.c2}>a</span>
      <span className={styles.c3}>l</span>
      <span className={styles.c4}>l</span>
      <span className={styles.c5}>o</span>
      <span className={styles.c6}>o</span>
      <span className={styles.c7}>n</span>
      <span className={styles.c8}> </span>
      <span className={styles.c9}>B</span>
      <span className={styles.c10}>l</span>
      <span className={styles.c11}>a</span>
      <span className={styles.c12}>s</span>
      <span className={styles.c13}>t</span>
    </h1>
    </div>
  );
}

export default Header;
